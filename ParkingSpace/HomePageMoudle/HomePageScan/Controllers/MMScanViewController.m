...
- (void)drawTitle
{
    if (!_tipTitle)
    {
        ...
        _tipTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, self.view.center.y - self.view.frame.size.width/2 );
        _tipTitle.font = [UIFont systemFontOfSize:13];
        _tipTitle.textAlignment = NSTextAlignmentCenter;
        _tipTitle.numberOfLines = 0;
        _tipTitle.text = @"对准二维码扫描";
...
- (void)drawBottomItems
{
    ...
    self.toolsView = [[UIView alloc]initWithFrame:CGRectMake(0, self.view.center.y + (SCREEN_WIDTH-120)/2.0 + 20,
                                                                   CGRectGetWidth(self.view.frame), 120)];
    ...
    title.font = kFontSize(30);
    title.textColor = [UIColor blackColor];
    title.textAlignment = NSTextAlignmentLeft;
    title.text = @"嗨泊停车";
    title.frame = CGRectMake(16, 27, SCREEN_WIDTH - 80,40);
    ...
    inputBtn.frame = CGRectMake(72, orginY, 56, 56);
    inputBtn.layer.cornerRadius = 28.0;
    inputBtn.clipsToBounds = YES;
    
    inputBtn.backgroundColor = [UIColor whiteColor];
    
    [inputBtn setImage:[UIImage imageNamed:@"手动输入泊位编号"] forState:UIControlStateNormal];
    [inputBtn addTarget:self action:@selector(_inputBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [_toolsView addSubview:inputBtn];
    
    
    UILabel * inputBtnLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, orginY+66, 120, 30)];
    inputBtnLabel.centerX = inputBtn.centerX;
    inputBtnLabel.text = @"手动输入泊位编号";
    inputBtnLabel.textAlignment = NSTextAlignmentCenter;
    inputBtnLabel.font = [UIFont systemFontOfSize:13];
    inputBtnLabel.textColor = [UIColor whiteColor];
    [_toolsView addSubview:inputBtnLabel];
    
    
    UIButton * torchBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    torchBtn.frame = CGRectMake(SCREEN_WIDTH - 72 - 56, orginY, 56, 56);
    torchBtn.backgroundColor = [UIColor colorWithHexString:@"#EDBD03"];
    
    torchBtn.layer.cornerRadius = 28.0;
    torchBtn.clipsToBounds = YES;
    
    [torchBtn setImage:[UIImage imageNamed:@"手电筒"] forState:UIControlStateNormal];
    [torchBtn addTarget:self action:@selector(openFlash:) forControlEvents:UIControlEventTouchUpInside];
    [_toolsView addSubview:torchBtn];
    
    UILabel * tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH- 120, orginY+66, 100, 30)];
    tipLabel.centerX = torchBtn.centerX;
    tipLabel.text = @"手电筒";
    tipLabel.textAlignment = NSTextAlignmentCenter;
    tipLabel.font = [UIFont systemFontOfSize:14];
    tipLabel.textColor = [UIColor whiteColor];
    [_toolsView addSubview:tipLabel];
}
...
-(void)_inputBtnClick:(UIButton *)sender
{
    ...
    [self dismissViewControllerAnimated:NO completion:^{
        if ([self.delegate respondsToSelector:@selector(qrScanInputBtnClick:viewController:)])
        {
            [self.delegate qrScanInputBtnClick:sender viewController:self];
        }
    }];
}
